...
module.exports.createTestData = async () => {
  ...
  const messages2User2 = await factory.createMany('Message', 5, { chatId: chat2.id, userId: user3.id, createdAt: () => faker.date.recent(400) })
  
  const messages3User1 = await factory.createMany('Message', 5, { chatId: chat3.id, userId: user1.id, createdAt: () => faker.date.recent(400) })
  const messages3User2 = await factory.createMany('Message', 5, { chatId: chat3.id, userId: user4.id, createdAt: () => faker.date.recent(400) })

  const messages4User1 = await factory.createMany('Message', 5, { chatId: chat4.id, userId: user1.id, createdAt: () => faker.date.recent(400) })
  const messages4User2 = await factory.createMany('Message', 5, { chatId: chat4.id, userId: user5.id, createdAt: () => faker.date.recent(400) })
};
